...
# --- 🚀 1. EKRAN: GİRİŞ SEÇİMİ (GÖRSEL ŞÖLEN) ---
...
# --- 👤 2. EKRAN: VATANDAŞ PORTALI ---
    ...
    if st.session_state.sayfa == "asistan_ana":
        ...
        with st.container(border=True):
            ...
            if ep and not re.match(EMAIL_PATTERN, ep, re.IGNORECASE): st.error("⚠️ E-posta düzgün formatta giriniz!")
            tel = c2.text_input("Telefon (05xx...)")
            if tel and not re.match(PHONE_PATTERN, tel): st.error("⚠️ Telefonu düzgün formatta giriniz!")
            ...
            if st.button("Talebi Gönder") and ad and soyad and re.match(EMAIL_PATTERN, ep) and re.match(PHONE_PATTERN, tel):
            ...
            if arama:
                ...
                if not df_m.empty:
                    ...
                    if not k_msg.empty:
                        ...
                        st.markdown("---")
                        with st.expander("📥 Buradan Yanıt Yaz / Belge Gönder"):
                            with st.form("query_reply"):
                                rm = st.text_area("Mesajınız:"); rf = st.file_uploader("Belge Ekle")
