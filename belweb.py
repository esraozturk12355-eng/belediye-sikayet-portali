...
# --- 👤 2. EKRAN: VATANDAŞ PORTALI ---
    ...
    if st.session_state.sayfa == "asistan_ana":
        ...
        with st.container(border=True):
            ...
            # Yorumun ismi "Vatandaş Açıklaması" olarak güncellendi
            det = st.text_area("Talebiniz / Mesajınız (Müdürlük bunu görecektir)")
            if st.button("Talebi Gönder", disabled=not (ad and soyad and m_valid and p_valid)):
                sid = str(time.time()).replace(".","")[-6:]
                pd.DataFrame([{
                    "ID": sid, "Tarih": tr_saat(), "Ad": ad, "Soyad": soyad, 
                    "E-posta": ep, "Telefon": tel_temizle(tel), "Müdürlük": mud_sec, 
    ...
    if adm_s == "1234":
        ...
        with t1:
            ...
            if not df_t.empty:
                ...
                if not filt.empty:
                    # MÜDÜRLÜK ARTIK "Vatandaş Açıklaması" SÜTUNUNU GÖREBİLECEK
