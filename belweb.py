...
if menu == "Yeni Şikayet Oluştur":
    ...
    if st.button("Şikayeti Kaydet"):
        if ad and soyad and eposta:
            ...
            # --- MÜDÜRLÜĞE ÖZEL SIRA NO HESAPLAMA ---
            yeni_sira_no = 1
            if not df_mevcut.empty and "Müdürlük" in df_mevcut.columns:
                ...
                if not birim_kayitlari.empty:
                    yeni_sira_no = birim_kayitlari["Sıra_No"].max() + 1
            ...
            yeni_kayit = {
                "ID": sikayet_id,
                "Sıra_No": yeni_sira_no, # Müdürlüğe özel sıra
                "Tarih": simdi,
    ...
    if arama:
        ...
        if not df.empty:
            ...
            if not sonuclar.empty:
                # Kullanıcı kendi sıra numarasını görebilir
...
if sifre == "1234":
    ...
    if not df_admin.empty:
        ...
        if not filtreli.empty:
            st.write(f"### {admin_birim} Şikayetleri (Sıralı)")
            # Tabloda Sıra_No en başa alındı
            ...
            with st.container():
                ...
                if st.button("Değişiklikleri Onayla"):
                    ...
                    if not idx.empty:
                        # Eğer müdürlük değişiyorsa, yeni müdürlüğün son sıra nosunu al
                        eski_mudurluk = df_admin.at[idx[0], "Müdürlük"]
                        if eski_mudurluk != yönlendir:
                            hedef_kayitlar = df_admin[df_admin["Müdürlük"] == yönlendir]
                            yeni_birim_sira = 1 if hedef_kayitlar.empty else hedef_kayitlar["Sıra_No"].max() + 1
                            df_admin.at[idx[0], "Sıra_No"] = yeni_birim_sira
            ...
            st.info(f"{admin_birim} için şikayet yok.")
